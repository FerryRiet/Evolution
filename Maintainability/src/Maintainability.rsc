...
void DoIt() {
      ...
      list[tuple[str,loc,int,int]] aresult = [] ; // <Name, location, lines, complexity>
...
int calcMethodLinesV2(M3 M3Model,loc cu) {
     ...
     modifiedContent = visit(content){
                 case /<word:\/\*(.|[\r\n])*\*\/>/ => "" 
                 case /<word:\/\/.*>/              => "" 
                //case /<word:^\s*$>/              => ""  // causes Rascal to loop with 100% cpu load
     };
...
// (I leave it in because it is much faster then my regex solution)
